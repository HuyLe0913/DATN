...
async def search_hybrid(query: str, symbol: str = None, year: int = None, quarter: str = None, limit: int = 3) -> List[SearchResult]:
    print(f"[TRACKING TOOL CALL] Executing Vector Search (ask_vector) for query: '{query}', symbol: {symbol}, year: {year}, quarter: {quarter}", flush=True)
...
async def retrieve_graph_context(query: str, symbol: Optional[str] = None) -> dict:
    print(f"[TRACKING TOOL CALL] Executing GraphRAG (ask_graph) for query: '{query}', symbol: {symbol}", flush=True)
    """
