    ...
    async def stream_agent_response():
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                async with client.stream(
                    ...
                    async for chunk in response.aiter_bytes():
                        # Parse chunk to collect final content for storage
                        # Note: This part depends on the exact format Agent Core sends.
                        # For now, we yield directly and try to find the 'FINAL:' part.
                        decoded_chunk = chunk.decode()
                        if "FINAL: " in decoded_chunk:
                            # Extract part after FINAL:
            ...
            if full_response_content:
                ...
                # Cập nhật title nếu là tin nhắn đầu (tùy chọn)
